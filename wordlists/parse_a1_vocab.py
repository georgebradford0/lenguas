...
def is_example_sentence(line):
    """Check if line is example sentence."""
    if re.match(r'^[A-ZÄÖÜ].*[.!?]$', line):
        return True
    if re.search(r'\s(ich|du|er|sie|es|wir|ihr|Sie|ist|sind|hat|haben)\s', line):
        return True
    return False
...
def parse_vocabulary(input_file):
    ...
    A1 format is compact - words are listed with minimal conjugation info,
    often just the word followed by example sentences.
    """
    ...
    while i < len(lines):
        ...
        # Find start of alphabetical list - look for "A" after section starts
        ...
        # Pattern 1: Nouns - "der/die/das Word, ..."
        # A1 format: "der Arzt, -Ä, e" or "die Adresse, -n"
        noun_match = re.match(r'^(der|die|das)\s+([A-ZÄÖÜ][a-zäöüß\-]+)', line)
        if noun_match:
        ...
        # Pattern 2: Verbs (A1 often lists just infinitive without full conjugation)
        # Look for lowercase word that's a verb infinitive
        verb_match = re.match(r'^([a-zäöü]+en)$', line)
        if verb_match:
        ...
        # Pattern 3: Compound verbs or expressions with "auf sein", "aus sein"
        ...
        # Pattern 4: Adjectives/adverbs (single lowercase word)
        if re.match(r'^[a-zäöüß]+(-[a-zäöüß]+)?$', line):
            pos = classify_pos(line)
        ...
        # Pattern 5: Common expressions (like "Auf Wiedersehen")
        if re.match(r'^[A-ZÄÖÜ][a-zäöüß]+\s+[A-ZÄÖÜ][a-zäöüß]+', line) and not is_example_sentence(line):
            entry = {
                "word": line,
                "article": "",
                "pos": "expression",
...
def main():
    input_file = 'a1_extracted.txt'
    output_file = 'a1_vocabulary.json'

    print("Parsing A1 vocabulary...")
    ...
    for pos in ['noun', 'verb', 'adjective', 'adverb', 'preposition', 'expression']:
